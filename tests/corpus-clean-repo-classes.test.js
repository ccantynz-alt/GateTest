...
describe('hardcodedUrl — a bare http://localhost is a URL-parse base, not an endpoint', () => {
  ...
  it('silent on the axios idioms', async () => {
    const f = await runModule(HardcodedUrl, {
      ...
      // The 2026-09-02 version of this test also expected `isAbsolute ? path
      // : 'http://localhost/'` to be silent. main's own control pair
      // (tests/hardcoded-url.test.js, 2026-09-05) holds that a bare localhost
      // in a non-env ternary is a leak, and that pair is the newer measured
      // decision — so the ternary is not asserted here either way.
    });
