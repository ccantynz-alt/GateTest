...
describe('code-quality — the nearest package.json owns the file', () => {
  ...
  it('a package with a compile step ships its OUTPUT — source outside `files` is still library code', () => {
    // hono-shaped: `files: ["dist"]`, `scripts.build`, `main: dist/cjs/index.js`.
    // The list names compiled output; src/ is what becomes it. Measured on
    // the pinned corpus clone 2026-09-06: honouring `files` here silenced 5
    // blocking console.log errors in hono and 1 in rails' actioncable.
    const root = makeRepo({
      'package.json': { name: 'hono', version: '1.0.0', main: 'dist/cjs/index.js', files: ['dist', '!dist/**/*.tsbuildinfo'], scripts: { build: 'tsc', test: 'vitest' } },
      'src/index.ts': LOGGING,
    });
    try {
      assert.strictEqual(severityOf(root, 'src/index.ts'), undefined, 'compiled-in source is the shipped artefact');
    } finally { fs.rmSync(root, { recursive: true, force: true }); }
  });

  it('CONTROL: without a compile step the same `files` list keeps src/ outside the package', () => {
    const root = makeRepo({
      'package.json': { name: 'hono', version: '1.0.0', main: 'dist/cjs/index.js', files: ['dist'], scripts: { test: 'vitest', prepublishOnly: 'npm test' } },
      'src/index.ts': LOGGING,
    });
    try {
      assert.strictEqual(severityOf(root, 'src/index.ts'), 'info', '`prepublishOnly` runs checks, not a compiler — the list stands');
    } finally { fs.rmSync(root, { recursive: true, force: true }); }
  });
