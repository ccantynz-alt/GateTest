...
function composePrBody(opts) {
  ...
  // Severity-weighted summary badge — top of the report so reviewers
  // immediately see the severity breakdown without scrolling.
  const origFindings = Array.isArray(originalFindingsByModule)
    ? originalFindingsByModule
    : Object.values(originalFindingsByModule || {}).flat();
  const sevErrors = origFindings.filter((f) => f && (f.severity === 'error' || f.passed === false && !f.severity)).length;
  const sevWarnings = origFindings.filter((f) => f && f.severity === 'warning').length;
  const sevInfo = origFindings.filter((f) => f && f.severity === 'info').length;
  const sevParts = [];
  if (sevErrors > 0) sevParts.push(`🔴 **${sevErrors} error${sevErrors !== 1 ? 's' : ''}**`);
  if (sevWarnings > 0) sevParts.push(`🟡 **${sevWarnings} warning${sevWarnings !== 1 ? 's' : ''}**`);
  if (sevInfo > 0) sevParts.push(`ℹ️ ${sevInfo} info`);
  if (sevParts.length > 0) {
    sections.push(sevParts.join(' · '));
    sections.push('');
  }
  ...
  sections.push('---');
  sections.push('');
  sections.push('<sub>Scanned and fixed by <a href="https://gatetest.io">GateTest</a> — 120 modules · AI-powered · verify-before-commit · per-scan pricing</sub>');
