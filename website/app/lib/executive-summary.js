...
function renderExecutiveSummary(result, { hostname } = {}) {
  ...
  lines.push('---');
  lines.push('');
  lines.push(`<sub>This executive summary is part of the <a href="${SITE_URL}">GateTest $399 Forensic</a> tier. It synthesises the per-finding diagnosis and cross-finding correlation reports into a single read for senior stakeholders.</sub>`);
  return lines.join('\n');
}
