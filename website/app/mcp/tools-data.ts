...
export const ALL_TOOLS = [
  ...
  { name: "run_live_checks", paid: false, desc: "👂 Ears — runtime errors, console warnings, API health on any URL" },
  { name: "get_production_errors", paid: false, desc: "👂 Ears — top Sentry / Datadog / Rollbar errors with file:line attribution" },
  { name: "verify_fix", paid: false, desc: "🤝 Hands — re-scan changed files for hard pass/fail proof the fix worked" },
  { name: "run_tests", paid: false, desc: "🤝 Hands — auto-detect + run the project's test suite (Jest/Vitest/pytest/cargo/go)" },
  { name: "stream_logs", paid: false, desc: "🤝 Hands — tail a running process or log file in real time (up to 60s)" },
  { name: "query_db", paid: false, desc: "🤝 Hands — read-only SQL/NoSQL queries (Postgres/MySQL/SQLite/MongoDB/Redis)" },
  { name: "http_request", paid: false, desc: "🤝 Hands — call any API with auth headers, follow redirects, inspect responses" },
  { name: "audit_log", paid: false, desc: "Query past local scans in the memory store" },
  { name: "compare_repos", paid: false, desc: "Cross-repo prior-art lookup via memory store" },
  { name: "get_report", paid: false, desc: "Retrieve full result of the last scan this session" },
  { name: "scan_repo", paid: false, desc: "Scan any public git repo via the hosted API" },
  { name: "resolve_stack_trace", paid: false, desc: "Minified/bundled stack trace → original file:line via source maps" },
  { name: "blame_regression", paid: false, desc: "Which git commit introduced this line — read-only, ranks candidates across a stack trace" },
];
