 ...
 * tests/compliance-mappings.test.js assert coverage + shape. The website
 * (website/app/lib/compliance-mappings.js) re-exports this file.
 *
...
const MODULE_COMPLIANCE = {
  ...
  hardcodedUrl: {
    // Misconfiguration first: a baked-in endpoint is an environment that
    // cannot be changed without a deploy. The access-control angle (an
    // internal host reachable from the wrong tier) is secondary.
  ...
  redos: {
    // A catastrophic regex is the application's own code, not an outdated
    // component (A06); OWASP files CWE-1333-class weaknesses under
    // misconfiguration. The SARIF reporter had A05 all along; the two lists
    // are one now.
    owasp: ['A05:2021'],
    soc2:  ['A1.1'],
    cis:   ['16'],
