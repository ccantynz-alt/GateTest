...
// './prompt-injection-guard', not '../website/app/lib/…'. lib/ ships in the npm
// package (package.json files) and website/ does not, so the website path was a
// MODULE_NOT_FOUND crash for every installed user — and this is the module that
// supplies the anti-injection preamble and output leak scan, i.e. the one whose
// absence is least acceptable. The identical implementation was already sitting
// in this directory; lib/nuclear-diagnoser.js has always required it correctly.
// Guarded by tests/lib-shims.test.js. See Known Issue #74.
const { ANTI_INJECTION_PREAMBLE, wrapUntrusted, scanOutputForLeaks } = require('./prompt-injection-guard');
// DEFAULT_MODEL resolves through engine-models so GATETEST_CHEAP_MODEL reaches
...
const { endpoint: anthropicEndpoint, apiPath: anthropicApiPath, apiVersion: anthropicVersion } = require('../src/core/anthropic-config');

// ── Constants ───────────────────────────────────────────────────────────────

// Resolved from anthropic-config so ANTHROPIC_BASE_URL / ANTHROPIC_VERSION
// reach this path too. Kept as exported constants because ai-ci-fixer-core
// re-exports them (KI #78).
const ANTHROPIC_API_HOST    = anthropicEndpoint().hostname;
const ANTHROPIC_API_PORT    = anthropicEndpoint().port;
const ANTHROPIC_API_PATH    = anthropicApiPath('/v1/messages');
const ANTHROPIC_VERSION     = anthropicVersion();
const DEFAULT_MODEL         = CHEAP_MODEL;
