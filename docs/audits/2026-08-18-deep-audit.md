...
| 5 | The 5-comment PR: budgeted, ranked, evidence-attached, never repeated | **complete 2026-08-25** — budget + ranking shipped 99c59379; evidence gate shipped: the ai-review prompt now REQUIRES a verbatim code quote per finding, src/core/ai-evidence-gate.js verifies it against the reviewed files (confirmed / relocated-with-corrected-line / rejected — models see bugs well and count lines badly, so a real quote at a wrong line survives with the line fixed), rejects surface as one auditable info line, and the verified quote travels through finding-registry into the PR comment as an evidence sub-line | complaints #3/#9/#22 |
...
| 13 | Suppression in place: 👎 on the PR writes `.gatetestignore` and feeds precision | **shipped 2026-08-25** (as a reply command — GitHub has no reaction webhook): an OWNER/MEMBER/COLLABORATOR replies `@gatetest ignore <rule>` under the PR comment and the webhook commits the rule to .gatetestignore on the PR head branch (idempotent, fork-refusing, grammar-validated against injection, bot-proof) + acks in-thread; the PR comment footer advertises the command; every suppression lands in the new rule_suppressions table; `issue_comment` declared in github-app-permissions.js + listing + Craig checklist 2b (live App must subscribe). Follow-up: hosted auto-demotion should READ rule_suppressions the way noise-model reads local dismissals | complaint #15 |
