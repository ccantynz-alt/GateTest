...
| Which packages does this file import? | `src/core/import-graph.js` `externals` — the bare specifiers left over once the graph has resolved everything internal, read from the same statements and the same masked text as the edges (`dependencyReachability` counts production imports from it; Vue / Svelte script blocks keep a regex fallback because they are not JavaScript files) |
