...
| `madge --circular` (standalone CLI, separate install, no gate integration) / `eslint-plugin-import/no-cycle` (opt-in, slow, TS-alias-blind) / `dependency-cruiser` (heavy config) / `tsc` catches nothing — nothing gate-native for JS+TS import cycles with suppression markers, and none of them elide type-only imports the way the compiler does, so a cycle through an `.interface.ts` barrel is reported as if it loaded (2026-09-05: GateTest classifies each import as type-only / load-time / deferred, blocks only load-time cycles, and warns on call-time-only ones as `cycle-deferred`) | `gatetest --module importCycle` |
| `dependency-cruiser` (can express layer rules, but you must hand-write and maintain the ruleset — it never tells you the grain your code already has) / `madge` (draws the graph, judges nothing) / SonarQube + CodeClimate report per-file complexity, which says a file is hard to READ, not what it costs to CHANGE / ESLint `import/no-restricted-paths` (needs a configured layer map per repo) / Structure101 + NDepend (paid, desktop, .NET/Java-centric) — nothing gate-native computes blast radius, INFERS the layering grain from the codebase so it needs no config, excludes test-only fan-in, calibrates thresholds per repo, and trends coupling scan-over-scan | `gatetest --module spineHealth` |
