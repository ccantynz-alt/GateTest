...
- [~] **2.4** Real-repo proof on 3 repos — **PARTIAL 1/3** (commit `(this commit)`). `docs/proofs/phase-2-self-pair-review-and-architecture.md` documents real Claude calls exercising both depth deliverables on this repo: pair-review scored a real fix 4/3/4/1 (correctly low test-coverage score caught the missing regression test); architecture annotator analysed 122 source files in 34s and flagged duplication between `github-bridge.js` and `gluecron-bridge.js`. 2 more proofs needed against external target repos Craig nominates.
...
- [~] **3.7** Real-repo proof on 3 repos — **PARTIAL 1/3** (commit `(this commit)`). `docs/proofs/phase-3-self-nuclear.md` documents real Claude calls exercising all three Nuclear deliverables on this repo's actual self-scan findings: 12/12 findings diagnosed, **4 real attack chains identified** (including a textbook weak-secret + localhost-URL session-forgery vector — the kind no per-finding scanner can see), executive summary produced with 3 specific file-level recommendations a CTO can read in 5 minutes. Bug found + fixed mid-run: section-header regex needed `:(\s|$)` instead of `:\s` to match end-of-line headers after `split('\n')`. 2 more proofs needed against external targets.
...
| 2 — $199 Scan + Fix tier | 2026-04-26 | 3/5 sub-tasks at scaffold-or-better (2.1 ✓, 2.2 ✓, 2.4 ~ partial 1/3 proof real-Claude validated; 2.3 still pending — needs external-repo proofs to fully unlock pre-auth) |
| 3 — $399 Nuclear tier | 2026-04-26 | 4/7 sub-tasks shipped (3.1 ✓, 3.2 ✓, 3.5 ✓, 3.7 ~ partial 1/3 proof real-Claude validated identifying 4 real attack chains; 3.3/3.4/3.6 open). 3.3 (mutation testing) and 3.4 (chaos/fuzz) need either a runtime (Vercel functions can't run customer test suites) or a new dependency (Stryker, etc.) — both Boss-Rule-adjacent so deferred to a session where Craig can confirm direction. |
