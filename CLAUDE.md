...
| `website/app/api/scan/run/route.ts` | The actual scan execution. Calls `runBridgeTier` (real 67-module CLI engine against /tmp) with `runTier` as catch-fallback. Must stay on `runtime = "nodejs"` and `maxDuration = 300` | Changing scan logic |
| `website/app/lib/cli-bridge/static-registry.ts` | 67 literal `require()`s that make Webpack trace every CLI module into the Vercel bundle. DO NOT convert to dynamic paths — bundle will silently drop modules | Adding a module (also add to capabilities.ts, update smoke test count) |
| `website/app/lib/cli-bridge/capabilities.ts` | Per-module serverless capability + honest skip reasons. fs-only / fs-with-optional-exec / needs-git run serverless; needs-toolchain / needs-browser surface a verbatim skipReason to the customer | Adding a module or changing a skip message |
| `website/app/lib/cli-bridge/materialize.ts` | Writes RepoFile[] into /tmp with path-traversal guards and 2 MB/file + 400 MB/scan caps | Changing the scratch filesystem contract |
| `website/app/lib/cli-bridge/adapt.ts` | TestResult JSON → ModuleResultEnvelope. Zero-check modules become honest `skipped`, never fake-pass | Changing what the frontend sees |
| `website/app/lib/cli-bridge/run.ts` | Orchestrator with 20 s per-module timeout and `finally { cleanup }` | Changing bridge orchestration |
| `website/tests/cli-bridge.test.js` | Tripwire on bridge/capability/registry drift. DO NOT weaken | Adding a module (bump EXPECTED_COUNT) |
| `website/next.config.ts` | `outputFileTracingRoot` + `outputFileTracingIncludes` for `/api/scan/run` — without this, CLI modules do not ship in the serverless bundle | Changing what gets traced into the scan bundle |
...
| 13 | Website scanned only 14 hand-written TS re-implementations; CLI shipped 67. Landing page / pricing / CLI help all claimed 67 — customers paid for 67 and got 14. Forbidden List #19 territory (marketing claimed work we didn't do). | CRITICAL | DONE (2026-04-17) — **CLI bridge** shipped under Craig's integrity-and-"no-loose-ends" directive. New `website/app/lib/cli-bridge/` with `static-registry.ts` (67 literal `require()`s so Webpack traces every module), `capabilities.ts` (per-module serverless declaration + honest skip reasons — 48 fs-only, 2 fs+optional-exec, 4 needs-git, 6 needs-toolchain, 5 needs-browser), `materialize.ts` (RepoFile[] → /tmp with traversal guards, 2 MB/file + 400 MB/scan caps), `adapt.ts` (TestResult → ModuleResultEnvelope, zero-check modules become honest `skipped` not fake-pass), `run.ts` (20 s per-module timeout, cleanup in finally). `/api/scan/run` now calls `runBridgeTier()` first with the TS registry as a loud-logged catch-fallback. `next.config.ts` gained `outputFileTracingRoot: path.resolve(__dirname, "..")` + `outputFileTracingIncludes: { "/api/scan/run": ["../src/**/*.js", "../src/**/*.json"] }`; route exports `runtime = "nodejs"` and `maxDuration = 300`. `MAX_FILES_TO_READ` raised 50 → 400. Health check `checkModules()` now asserts 67 bridge entries + capability parity; `checkLiveScan()` exercises the bridge against fixture files and asserts 67 envelopes returned. New `website/tests/cli-bridge.test.js` (7 tests) is the tripwire: 67 literal requires / no dynamic paths / 67 capability entries / registry ↔ capability parity / valid capability categories / 11 toolchain+browser entries carry specific skipReasons / every bridge name exists in `src/core/registry.js` BUILT_IN_MODULES. Sweep green: 882/882 tests (864 → 882), 67/67 modules load, `next build` clean. |
