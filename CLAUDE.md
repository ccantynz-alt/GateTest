...
- [~] **2.4** Real-repo proof on 3 repos — **PARTIAL 2/3**. (1) `docs/proofs/phase-2-self-pair-review-and-architecture.md` documents the gatetest self-proof. (2) `docs/proofs/phase-2-3-crontech-real-customer-grade.md` documents the Crontech proof: real customer-grade target (multi-tenant Bun+Turbo TypeScript monorepo) scanned cleanly via the CLI — 23/39 modules passed, 754 errors, 1617 warnings in 25.5s. 1 more external-repo proof still needed.
...
- [~] **3.7** Real-repo proof on 3 repos — **PARTIAL 2/3**. (1) `docs/proofs/phase-3-self-nuclear.md` documents the gatetest self-proof: 12/12 findings diagnosed, 4 real attack chains, including a textbook weak-secret + localhost-URL session-forgery vector. Bug found + fixed mid-run (section-header regex). (2) `docs/proofs/phase-2-3-crontech-real-customer-grade.md` documents the Crontech proof: 10/10 findings diagnosed, **2 CRITICAL attack chains** identified in real customer-grade production code (frontend-bundle credential exposure + supply-chain CI-credential takeover), executive summary clean. ~$1 Anthropic spend per Nuclear scan against a 10-finding sample — comfortable margin at $399 tier price. 1 more external-repo proof still needed.
...
| 2 — $199 Scan + Fix tier | 2026-04-26 | 3/5 at scaffold-or-better (2.1 ✓, 2.2 ✓, 2.4 ~ partial 2/3 proofs validated incl. Crontech real-customer-grade run; 2.3 needs 1 more external-repo proof to fully unlock pre-auth). |
| 3 — $399 Nuclear tier | 2026-04-26 | 4/7 shipped (3.1 ✓, 3.2 ✓, 3.5 ✓, 3.7 ~ partial 2/3 proofs validated incl. Crontech identifying 2 CRITICAL chains in customer-grade code; 3.3/3.4/3.6 open). 3.3 (mutation testing) and 3.4 (chaos/fuzz) need Stryker-vs-inline call from Craig. |
